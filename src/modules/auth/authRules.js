...
const isAuthenticated = rule()(async (parent, args, { me }, info) => {
   console.log('PERMISSION: isAuthenticated >', me);
   return me !== undefined;
});
...
const isOwner = rule()(async (parent, args, { me }, info) => {
   console.log('PERMISSION: isOwner >', me, args, parent);
...
const isCollaborator = model =>
   rule()(async (parent, args, { models }, info) => {
      console.log('isCollaborator 1', model);
      const result = await models[model]
         .findById('5c432fc72b20aa705f111003')
         .populate('projectId');
      console.log('isCollaborator 2', result);
      return true; //ctx.user.role === 'editor'
   });

export { isAuthenticated, isAdmin, isOwner, isCollaborator };
